...
public class Starter {
    ...
    public static BarSeries loadCsvSeries(String filename) {
        try {
            ...
            final BarSeries series = new BaseBarSeriesBuilder().withNumFactory(DoubleNumFactory.getInstance()).withName("AAPL").build();
            new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))
                    ...
                    .forEach(st -> {
                        ...
                        series.addBar(new BaseBar(Duration.ofDays(1), date.toInstant(), DoubleNum.valueOf(open), DoubleNum.valueOf(high), DoubleNum.valueOf(low), DoubleNum.valueOf(close), DoubleNum.valueOf(volume), DoubleNum.valueOf(close * volume), 1));
                    });
            return series;
        } catch (Exception e) {
            e.printStackTrace();
