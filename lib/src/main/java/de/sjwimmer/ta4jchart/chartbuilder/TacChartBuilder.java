...
public class TacChartBuilder {
    ...
    private JFreeChart createCandlestickChart(final BarSeries series) {
        ...
        candlestickRenderer.setAutoWidthGap(1);
        candlestickRenderer.setDrawVolume(false);
        candlestickRenderer.setDefaultItemLabelsVisible(false);
        candlestickRenderer.setUpPaint(new Color(0x35A360));  // Green up candle
        candlestickRenderer.setDownPaint(new Color(0xFF4D4D));
    ...
    public TacChartBuilder withIndicator(IndicatorConfiguration.Builder<?> indicatorConfigurationBuilder) {
        ...
        final int plotIndex = indicatorConfiguration.getPlotIndex();

        if (plotType == PlotType.OVERLAY) {
            assert plotIndex < combinedDomainPlot.getSubplots().size();
            if (chartType == ChartType.LINE) {
                ...
                final XYPlot candlestickPlot = (XYPlot) combinedDomainPlot.getSubplots().get(plotIndex);
    ...
    private XYLineAndShapeRenderer createLineRenderer(IndicatorConfiguration<?> indicatorConfiguration) {
        ...
        lineRenderer.setSeriesStroke(0, new BasicStroke(1.1f));
        return lineRenderer;
    }
